...
<template>
    <v-container class="mb-4">
        ...
        <v-row v-if="skinNotFound" justify="center" class="my-8">
            <v-col cols="12" md="6" class="text-center">
                <v-alert type="warning" variant="tonal">
                    {{ $t("This skin could not be found. It may have been deleted, or the link may be incorrect.") }}
                </v-alert>
                <back-link to="/skins">
                    <v-btn class="mt-4" color="primary" prepend-icon="mdi-arrow-left">
                        {{ $t("Back to Gallery") }}
                    </v-btn>
                </back-link>
            </v-col>
        ...
        <template v-else>
            ...
            <v-row v-if="skinMeta && skinMeta.description" class="mt-4">
                <v-col class="text-medium-emphasis">
                    <v-icon icon="mdi-creation" size="small"/>
                    {{ skinMeta.description }}
                    <span class="text-low-emphasis text-caption">&bull; Generated by AI</span>
                </v-col>
            </v-row>
...
const skinNotFound = computed(() => {
    return skinStatus.value === 'success' && !skin.value;
});
